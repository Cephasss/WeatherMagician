...
def patchify(input, patch_size):
    ...
    # patches = F.unfold(input, kernel_size=patch_size, stride=patch_size).permute(0,2,1)
    # C = input.size(1)  # Number of channels
    # patches = patches.view(-1, C, patch_size * patch_size)  # Shape: [N*L, C, patch_size*patch_size]
