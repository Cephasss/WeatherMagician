...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        if depth is not None:
            self.dam_depth = depth.clamp(0.0, 1.0)[0,:,:]
            self.dam_depth = self.dam_depth.unsqueeze(0)
            mask = torch.ones((1, self.image_height, self.image_width))

        if gt_alpha_mask is not None:
            self.original_image *= gt_alpha_mask
            self.dam_depth *= gt_alpha_mask
            # .to(self.data_device)
        else:
            self.original_image *= torch.ones((1, self.image_height, self.image_width))
                                                #   , device=self.data_device)
            if depth is not None:
                self.dam_depth *= torch.ones((1, self.image_height, self.image_width))
                                                #   , device=self.data_device)
            else:
                self.dam_depth = None
        self.mask = mask
